...
# Function to calculate MACD (Fixed)
...
# Function to detect trade setup (with adjusted thresholds and fixed MACD crossover)
def detect_trade_setup(candle, near_support, near_resistance, price_history, channel_range, params):
    ...
    if channel_range < 0.0008:  # Lowered from 0.0015
    ...
    if atr is None or atr < 0.0002:  # Lowered from 0.0003
    ...
    for i in range(1, min(4, len(closes) - 26)):
        # Calculate previous MACD
        ema_12_prev = calculate_ema(closes[:-i], 12)
        ema_26_prev = calculate_ema(closes[:-i], 26)
        if ema_12_prev is None or ema_26_prev is None:
            continue
        prev_macd = ema_12_prev - ema_26_prev

        # Calculate previous signal line
        macd_values_prev = []
        for j in range(len(closes) - 26 - i, len(closes) - i):
            if len(closes[:j+1]) < 26:  # Ensure enough data for EMA_26
                continue
            ema_12 = calculate_ema(closes[:j+1], 12)
            ema_26 = calculate_ema(closes[:j+1], 26)
            if ema_12 is None or ema_26 is None:
                continue
            macd_values_prev.append(ema_12 - ema_26)
        if len(macd_values_prev) < 9:  # Need at least 9 values for signal line
            continue
        prev_signal = calculate_ema(macd_values_prev, 9)
        if prev_signal is None:
            continue

        # Calculate current MACD
        ema_12_curr = calculate_ema(closes[:-(i-1)], 12)
        ema_26_curr = calculate_ema(closes[:-(i-1)], 26)
        if ema_12_curr is None or ema_26_curr is None:
            continue
        curr_macd = ema_12_curr - ema_26_curr

        # Calculate current signal line
        macd_values_curr = []
        for j in range(len(closes) - 26 - (i-1), len(closes) - (i-1)):
            if len(closes[:j+1]) < 26:
                continue
            ema_12 = calculate_ema(closes[:j+1], 12)
            ema_26 = calculate_ema(closes[:j+1], 26)
            if ema_12 is None or ema_26 is None:
                continue
            macd_values_curr.append(ema_12 - ema_26)
        if len(macd_values_curr) < 9:
            continue
        curr_signal = calculate_ema(macd_values_curr, 9)
    ...
    print(f"[{time.strftime('%H:%M:%S')}] RSI: {rsi:.2f}, Overbought: {overbought}, Oversold: {oversold}, 20-SMA: {sma_20:.5f}, 50-SMA: {sma_50:.5f}, Trend Up: {trend_up}, Trend Down: {trend_down}, MACD: {macd_line:.5f}, Signal: {signal_line:.5f}")

    # Stricter trade logic
    if body < 0.0001:
        if near_resistance and trend_down and overbought and macd_cross_down:
            return "put"
        if near_support and trend_up and oversold and macd_cross_up:
            return "call"

    # Stricter pin bar
    if body < 0.0001 and total_range > 2 * body:
        if wick_high > 2 * body and wick_high > wick_low and near_resistance and trend_down and overbought and macd_cross_down:
            return "put"
        if wick_low > 2 * body and wick_low > wick_high and near_support and trend_up and oversold and macd_cross_up:
            return "call"
    return None
