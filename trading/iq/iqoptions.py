...
# Function to detect trade setup (relaxed pin bar + channel)
def detect_trade_setup(candle, near_upper, near_lower):
    ...
    # Relaxed criteria: Small body, or pin bar with dominant wick
    if body < 0.0001:  # Your trades often have very small bodies
        if near_upper:
            return "put"
        if near_lower:
            return "call"
    # Classic pin bar
    if body < 0.0004 and total_range > body:
        if wick_high > 1.5 * body and wick_high > wick_low and near_upper:
            return "put"
        if wick_low > 1.5 * body and wick_low > wick_high and near_lower:
            return "call"
    return None
