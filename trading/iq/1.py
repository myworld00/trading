...
trade_amount_input = "1%"  # 1% of balance
...
def calculate_trade_amount(balance, trade_amount_input):
    if trade_amount_input.endswith("%"):
        ...
        # Ensure trade amount is at least $1 and capped at $5
...
# Function to detect trade setup (with improved trend and volatility filter)
def detect_trade_setup(candle, near_upper, near_lower, price_history, channel_range):
    ...
    if body < 0.0002:
        if near_upper:
            if strong_up_momentum or trend_up:
                return "call"
            return "put"
        if near_lower:
            if strong_down_momentum or trend_down:
                return "put"
            return "call"

    # Classic pin bar
    if body < 0.0006 and total_range > body:
        if wick_high > 1.2 * body and wick_high > wick_low and near_upper and not trend_up:
            return "put"
        if wick_low > 1.2 * body and wick_low > wick_high and near_lower and not trend_down:
            return "call"
    return None
    ...
    while True:
        ...
        if candle_time != last_candle_time:
            ...
            if len(price_history) >= 10:
                ...
                if direction:
                    # Update balance and trade amount before each trade
                    balance = api.get_balance()
                    trade_amount = calculate_trade_amount(balance, trade_amount_input)
                    ...
                    wick_high = latest_candle['max'] - max(latest_candle['open'], candle['close'])
                    wick_low = min(candle['open'], candle['close']) - candle['min']
